...
@tool
def get_historical_stock_prices(ticker:str)->str:
    """
    Fetches weekly historical stock prices data for a given company ticker for the last 90 days.
    Args:
        ticker (str): The Stock ticker symbol (e.g., "NVIDIA", "AMD")

    Returns:
        str: A string with lines of the date and weekly close price in the last 90 days.
    """
    try:
        ...
        daily_history = stock.history(
            start = start_date_str , end=end_date_str,interval="1d"
        )
        ...
        for date,close in zip[tuple](weekly_resampled.index,weekly_resampled):
        ...
        return f"An error has occured while fetching historical price for '{ticker}' : {e}"
...
def fetch_sec_filing_sections(ticker:str,sections: list[Section])->str:
    ...
    Args:
        ticker (str): The ticker symbol of the company to fetch the sec filing for.
        sections (list[Section]): The sections to fetch from SEC filing. Available sections are (mda, risk_factors, balance_sheet, income_statement, cashflow_statement)
